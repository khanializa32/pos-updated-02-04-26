...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        //validamos que no sea un venta suspendida
        $payment_method = null;
        if ($input['is_suspend'] == '') {
        ...
        $transaction = Transaction::create([
            ...
            'e_invoice' => $invoice_e,
            'payment_method' => $payment_method,//Linea para guardar el metodo de pago cuando la venta es a credito
            'ref_no' => '',
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                if (!empty($product['lot_no_line_id'])) {
                    // Manual lot selection (when lot number tracking is enabled)
                    ...
                    // FEFO: auto-assign the earliest-expiry batch with available stock.
                    // Only runs when lot number tracking is OFF — otherwise the user picks the batch manually.
                    $fefo_business_id = $business_id ?? request()->session()->get('user.business_id');
                    $fefo_batch = $this->getFefoLotForSale(
                    ...
                    if ($fefo_batch) {
                        $line['lot_no_line_id'] = $fefo_batch->purchase_line_id;
                    } elseif (request()->session()->get('business.on_product_expiry') == 'stop_selling') {
                        // All available batches are expired or out of stock — block sale
                        throw new \Exception(
                            __('lang_v1.product_expired_cannot_sell', ['product' => $product['product_id']])
                        );
                    }
                }
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        if ($status_before != 'draft') {
            ...
            // Use per-line location if available, otherwise use transaction location
            $adjust_location_id = !empty($sell_line->location_id) ? $sell_line->location_id : $location_id;
            $this->adjustQuantity($adjust_location_id, $product['product_id'], $product['variation_id'], $difference);
        }
    ...
    /**
     * Returns the earliest-expiry purchase line with available stock for FEFO deduction.
     * Batches without an exp_date are prioritised first (treated as non-expiring).
     * Respects the business stop_selling_before setting.
     *
     * @param  int  $variation_id
     * @param  int  $business_id
     * @param  int  $location_id
     * @return object|null
     */
    public function getFefoLotForSale($variation_id, $business_id, $location_id)
    {
        $stop_selling_expired = request()->session()->get('business.on_product_expiry') == 'stop_selling';
        $stop_before = (int) request()->session()->get('business.stop_selling_before', 0);
        $cutoff_date = \Carbon::today()->addDays($stop_before)->toDateString();
        ...
        if ($stop_selling_expired) {
            // Exclude batches that are expired (or will expire within stop_selling_before days)
            $query->where(function ($q) use ($cutoff_date) {
                $q->whereNull('purchase_lines.exp_date')
                    ->orWhereRaw('purchase_lines.exp_date > ?', [$cutoff_date]);
            });
        }

        // FEFO order: batches with no exp_date first, then earliest expiry date
        $query->orderByRaw('CASE WHEN purchase_lines.exp_date IS NULL THEN 0 ELSE 1 END ASC')
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true, $invoice_scheme_id)
    {
        ...
        if (empty($sell_return_data['invoice_no']) && empty($sell_return)) {
            ...
            if ($res['id'] == 3) {
                ...
                $sell_return_data['invoice_scheme_id'] = $invoice_scheme_id;
