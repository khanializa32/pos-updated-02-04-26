...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($for == 'view_product' && !empty(request()->input('product_id'))) {
                ...
                // Fetch batch-level details (lot number, expiry, qty available) for this product
                $view_product_id = request()->input('product_id');
                $batch_details = \App\PurchaseLine::join('transactions as T', 'purchase_lines.transaction_id', '=', 'T.id')
                    ...
                    ->whereRaw('(purchase_lines.quantity - purchase_lines.quantity_sold - purchase_lines.quantity_adjusted - purchase_lines.quantity_returned) > 0')
                    ->where('v.product_id', $view_product_id)
                    ->select(
                    ...
                    ->orderByRaw('CASE WHEN purchase_lines.exp_date IS NULL THEN 0 ELSE 1 END ASC')
                    ->orderBy('purchase_lines.exp_date', 'asc')
                    ->get();

                return view('product.partials.product_stock_details')->with(compact('product_stock_details', 'batch_details'));
            }
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        $end_date = !empty(request()->input('stock_date')) ? $this->transactionUtil->uf_date(request()->input('stock_date')) : \Carbon::now()->format('Y-m-d');
        $location_id = request()->input('location_id');
        $filters = request()->only(['category_id', 'sub_category_id', 'brand_id', 'unit_id', 'stock_date']);
