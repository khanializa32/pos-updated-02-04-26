...
<tr class="product_row" data-row_index="<?php echo e($row_count, false); ?>" data-unit-discount="<?php echo e($product->unit_discount ?? '', false); ?>"
	<?php if(!empty($so_line)): ?> data-so_id="<?php echo e($so_line->transaction_id, false); ?>" <?php endif; ?>>
	<?php if(!empty($is_serial_no)): ?>
	...
	<td>
		...
		<?php if(($edit_price || $edit_discount) && empty($is_direct_sell)): ?>
			<div title="<?php echo app('translator')->get('lang_v1.pos_edit_product_price_help'); ?>" style="display: inline; font-size: 16px">
				<span class="text-link text-dark cursor-pointer" data-toggle="modal"
		...
		<?php else: ?>
					...
					<?php endif; ?>" alt="product-img" loading="lazy"
			style="height: 30%;display: inline;margin-left: 3px; border: black;border-radius: 3px; margin-top: 3px; width: 30px;object-fit: cover;">

		<input type="hidden" class="enable_sr_no" value="<?php echo e($product->enable_sr_no, false); ?>">
		<input type="hidden" name="products[<?php echo e($row_count, false); ?>][line_location_id]" class="line_location_id"
			value="<?php echo e($transactionLocation->id ?? '', false); ?>">
		...
		<small class="p-1" style="color: purple; font-weight: bold; font-size: 1.1rem;">
    ...
    <?php endif; ?>
		...
		<?php if(session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1): ?>
			...
			<?php if(!empty($product->lot_numbers) && empty($is_sales_order)): ?>
					...
					<select class="form-control lot_number input-sm" name="products[<?php echo e($row_count, false); ?>][lot_no_line_id]"
						...
						<?php $__currentLoopData = $product->lot_numbers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lot_number): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							...
							<option value="<?php echo e($lot_number->purchase_line_id, false); ?>" data-qty_available="<?php echo e($lot_number->qty_available, false); ?>"
								...
								<?php endif; ?> <?php if($lot_enabled == 1 && $exp_enabled == 1): ?> - <?php endif; ?>
								<?php if($exp_enabled == 1 && !empty($lot_number->exp_date)): ?> <?php echo app('translator')->get('product.exp_date'); ?>:
								<?php echo e(\Carbon::createFromTimestamp(strtotime($lot_number->exp_date))->format(session('business.date_format')), false); ?> <?php endif; ?> <?php echo e($expiry_text, false); ?>
				...
				<?php else: ?>
					...
					<?php
						$fefo_batch = $product->lot_numbers
							->where('qty_available', '>', 0)
							->sortBy(function ($l) {
								// nulls (no exp_date) come first, then sort by earliest
								return $l->exp_date === null ? '0000-00-00' : $l->exp_date;
							})
							->first();
					?>
					<?php if(!empty($fefo_batch)): ?>
						<small class="text-muted">
							<?php if($exp_enabled == 1 && !empty($fefo_batch->exp_date)): ?>
								<?php
									$fefo_is_expired = \Carbon::now()->gt(\Carbon::createFromFormat('Y-m-d', $fefo_batch->exp_date));
								?>
								<i class="fa fa-calendar"></i>
								<?php echo app('translator')->get('product.exp_date'); ?>: <?php echo e(\Carbon::createFromTimestamp(strtotime($fefo_batch->exp_date))->format(session('business.date_format')), false); ?>

								<?php if($fefo_is_expired): ?>
									<span class="label label-danger"><?php echo app('translator')->get('report.expired'); ?></span>
								<?php endif; ?>
							<?php elseif($exp_enabled == 1): ?>
								<i class="fa fa-calendar"></i> <?php echo app('translator')->get('lang_v1.no_expiry_date'); ?>
							<?php endif; ?>
						</small>
					<?php endif; ?>
				<?php endif; ?>
		...
		<?php if(!empty($is_direct_sell)): ?>
			...
			<textarea class="form-control" name="products[<?php echo e($row_count, false); ?>][sell_line_note]"
				rows="2"><?php echo e($sell_line_note, false); ?></textarea>
	...
	<td>
		...
		<div class="input-group input-number">
			...
			<input type="text" data-min="1" style="width: 6vh; padding: 0px 2px 0px 4px"
				...
				value="<?php echo e(number_format($product->quantity_ordered, session('business.quantity_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>" name="products[<?php echo e($row_count, false); ?>][quantity]"
				data-alert-quantity="<?php echo e($product->alert_quantity ?? '', false); ?>"
				data-qty_available="<?php echo e($product->qty_available ?? '', false); ?>"
				data-enable-stock="<?php echo e($product->enable_stock ?? '', false); ?>"
				...
				<?php if($product->enable_stock && empty($pos_settings['allow_overselling']) && empty($is_sales_order)): ?>
					data-rule-max-value="<?php echo e($max_qty_rule, false); ?>" data-msg-max-value="<?php echo e($max_qty_msg, false); ?>"
		...
		<div style="display: flex; align-items: center; gap: 5px; margin-top: 5px;">
			...
			<?php if(count($sub_units) > 0): ?>
				<select name="products[<?php echo e($row_count, false); ?>][sub_unit_id]" class="form-control input-sm sub_unit" style="width: 100%;">
					<option value="" data-multiplier="1" data-unit_name="" data-allow_decimal="1">Unidad</option>
					<?php $__currentLoopData = $sub_units; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
						<option value="<?php echo e($key, false); ?>" data-multiplier="<?php echo e($value['multiplier'], false); ?>" data-unit_name="<?php echo e($value['name'], false); ?>"
							data-allow_decimal="<?php echo e($value['allow_decimal'], false); ?>" data-price="<?php echo e($value['price'] ?? '', false); ?>"
							data-cost-price="<?php echo e($value['cost_price'] ?? '', false); ?>" <?php if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key): ?> selected <?php endif; ?>>
			...
			<?php else: ?>
				<span><?php echo e($product->unit, false); ?></span>
			<?php endif; ?>

			<i class="glyphicon glyphicon-trash text-danger pos_remove_row cursor-pointer" aria-hidden="true"
				style="font-size:24px;" title="Eliminar"></i>
		</div>
		...
		<?php if(!empty($product->second_unit)): ?>
			...
			<input type="text" name="products[<?php echo e($row_count, false); ?>][secondary_unit_quantity]"
				value="<?php echo e(number_format($product->secondary_unit_quantity, session('business.quantity_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>" class="form-control input-sm input_number"
				required>
		<?php endif; ?>
		...
		<input type="hidden" class="hidden_base_unit_sell_price_inc_tax"
			value="<?php echo e($product->sell_price_inc_tax / $multiplier, false); ?>">
		<?php if(empty($is_direct_sell)): ?>
			<?php echo Form::select("products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount_type, ['class' => 'form-control row_discount_type', 'style' => 'display:none;']); ?>

			<?php echo Form::hidden("products[$row_count][line_discount_amount]", number_format($discount_amount, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), ['class' => 'row_discount_amount']); ?>
			...
			<?php echo Form::select("products[$row_count][tax_id]", $tax_dropdown['tax_rates'], $tax_id, ['placeholder' => 'Select', 'class' => 'form-control tax_id', 'style' => 'display:none;'], $tax_dropdown['attributes']); ?>
	...
	<?php if(!empty($is_direct_sell)): ?>
		...
		<td class="<?php if(!auth()->user()->can('edit_product_price_from_sale_screen')): ?> hide <?php endif; ?>">
			<input type="text" name="products[<?php echo e($row_count, false); ?>][unit_price]"
				...
				data-rule-min-value="<?php echo e($product->default_purchase_price, false); ?>"
				data-msg-min-value="<?php echo e(__('lang_v1.cost_price_validation_error', ['cost_price' => number_format($product->default_purchase_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator'])]), false); ?>"
				data-cost-price="<?php echo e($product->default_purchase_price, false); ?>">

			<?php if(!empty($last_sell_line)): ?>
				<br>
				<small class="text-muted"><?php echo app('translator')->get('lang_v1.prev_unit_price'); ?>:
					<?php 
            $formated_number = "";
		...
		<td <?php if(!$edit_discount): ?> class="hide" <?php endif; ?>>
			...
			<?php if(!empty($discount)): ?>
				<p class="help-block">
					<?php echo __('lang_v1.applied_discount_text', ['discount_name' => $discount->name, 'starts_at' => $discount->formated_starts_at, 'ends_at' => $discount->formated_ends_at]); ?>
	...
	<td class="<?php echo e($hide_tax, false); ?>">
		<input type="text" style="width: 10vh" name="products[<?php echo e($row_count, false); ?>][unit_price_inc_tax]"
			...
			<?php if(!$edit_price): ?> readonly <?php endif; ?> data-rule-min-value="<?php echo e($product->default_purchase_price, false); ?>"
			data-msg-min-value="<?php echo e(__('lang_v1.cost_price_validation_error', ['cost_price' => number_format($product->default_purchase_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator'])]), false); ?>"
			data-cost-price="<?php echo e($product->default_purchase_price, false); ?>">
	</td>
	...
	<td class="text-center">
		<input type="hidden" style="width: auto" name="products[<?php echo e($row_count, false); ?>][unit_price_inc_tax]"
			class="form-control pos_unit_price_inc_tax input_number" value="<?php echo e(number_format($unit_price_inc_tax, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>"
			<?php if(!$edit_price): ?> readonly <?php endif; ?> data-rule-min-value="<?php echo e($product->default_purchase_price, false); ?>"
			data-msg-min-value="<?php echo e(__('lang_v1.cost_price_validation_error', ['cost_price' => number_format($product->default_purchase_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator'])]), false); ?>"
			data-cost-price="<?php echo e($product->default_purchase_price, false); ?>">
		...
		?>
		<input style="width: 9vh" type="<?php echo e($subtotal_type, false); ?>"
...
</tr><?php /**PATH /home/u371716713/domains/ziscoplus.com/public_html/alizazip/resources/views/sale_pos/product_row.blade.php ENDPATH**/ ?>