            ...
            success: function(result) {
                if(result) {
                    try {
                        ...
                        $('#purchase_entry_table > tbody:last-child').prepend(result);
                    }
                }
        },
        });
...
$("#saveProducts").on("click" , function(e){
    ...
    $("#purchase_entry_table tr:not([data-skip])").each(function () {
        ...
        if(this.id > 0){
            // 1. Obtenemos el valor como string primero
            amountAfterInventory = $("#productAfterInventory_"+variation_id).val();

            // 2. CORRECCIÓN: Validamos que no sea un string vacío, 
            // pero permitimos que sea "0" o mayor.
            if(amountAfterInventory === "" || amountAfterInventory === null){
                amountAfterInventoryFlag = true;
            }

            var amountDifference = $("#difference_"+variation_id).text();
            var data = {
                inventory_id: last_part, // Asegúrate que last_part esté accesible
                product_id: productId,
                amountAfterInventory: amountAfterInventory, // Se envía el valor capturado
    ...
    if(amountAfterInventoryFlag){
        // Este mensaje ahora solo saldrá si hay campos VACÍOS
...
</script><?php /**PATH /home/u371716713/domains/ziscoplus.com/public_html/alizazip/Modules/InventoryManagement/Providers/../Resources/views/partials/mainscript.blade.php ENDPATH**/ ?>