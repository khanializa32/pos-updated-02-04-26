			...
			<div class="row">
				<div class="form-group col-xs-12 <?php if(!auth()->user()->can('edit_product_price_from_sale_screen')): ?> hide <?php endif; ?>">
                    ...
                    <label><?php echo app('translator')->get('sale.unit_price'); ?></label>
                        <input type="text" name="products[<?php echo e($row_count, false); ?>][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="<?php echo e(number_format($pos_unit_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>" data-rule-min-value="<?php echo e($product->default_purchase_price, false); ?>" data-msg-min-value="<?php echo e(__('lang_v1.cost_price_validation_error', ['cost_price' => number_format($product->default_purchase_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator'])]), false); ?>" data-cost-price="<?php echo e($product->default_purchase_price, false); ?>" readonly>
				</div>
...
<?php /**PATH /home/u371716713/domains/ziscoplus.com/public_html/alizazip/resources/views/sale_pos/partials/row_edit_product_price_modal.blade.php ENDPATH**/ ?>