$(document).ready(function () {
    ...
    function __reset_unit_discount_modal() {
        $('#unit_discount_unit_id').val('');
        $('#unit_discount_base_unit_name').text('-');
        $('#unit_discount_amount').val('');
        $('#unit_discount_amount_error').text('').hide();
        $('#unit_discount_delete_btn').prop('disabled', true);
    }

    function __validate_unit_discount_amount() {
        var rawValue = ($('#unit_discount_amount').val() || '').toString().trim();
        rawValue = rawValue.replace(/,/g, '');

        if (rawValue === '') {
            return { valid: false, message: 'Discount amount is required.' };
        }

        if (!/^\d+(\.\d{1,4})?$/.test(rawValue)) {
            return { valid: false, message: 'Please enter a valid discount amount.' };
        }

        if (parseFloat(rawValue) < 0) {
            return { valid: false, message: 'Discount amount must be 0 or greater.' };
        }

        return { valid: true, normalized: rawValue };
    }

    $(document).on('click', 'button.add_discount_button', function () {
        __reset_unit_discount_modal();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success === true && result.data) {
                    $('#unit_discount_unit_id').val(result.data.id);
                    $('#unit_discount_base_unit_name').text(result.data.base_unit_name || '-');
                    $('#unit_discount_amount').val(result.data.discount === null ? '' : result.data.discount);
                    $('#unit_discount_delete_btn').prop('disabled', result.data.discount === null || result.data.discount === '');
                    $('#unit_promotion_modal').modal('show');
                } else {
                    toastr.error(result.msg || LANG.some_error_in_input_field);
                }
            },
            error: function () {
    ...
    $(document).on('hidden.bs.modal', '#unit_promotion_modal', function () {
        __reset_unit_discount_modal();
    });

    $(document).on('submit', 'form#unit_discount_form', function (e) {
        e.preventDefault();

        var validation = __validate_unit_discount_amount();
        if (!validation.valid) {
            $('#unit_discount_amount_error').text(validation.message).show();
            return;
        }

        $('#unit_discount_amount_error').text('').hide();

        var unitId = $('#unit_discount_unit_id').val();
        if (!unitId) {
            toastr.error(LANG.some_error_in_input_field);
            return;
        }
        ...
        $.ajax({
            ...
            data: data,
            beforeSend: function (xhr) {
                __disable_submit_button($('#unit_discount_submit_btn'));
                __disable_submit_button($('#unit_discount_submit_icon_btn'));
            },
            success: function (result) {
                if (result.success === true) {
                    $('#unit_promotion_modal').modal('hide');
                    toastr.success(result.msg);
                    units_table.ajax.reload();
                } else {
                    var fieldError =
                        result.errors && result.errors.discount && result.errors.discount.length
                            ? result.errors.discount[0]
                            : null;

                    if (fieldError) {
                        $('#unit_discount_amount_error').text(fieldError).show();
                    }
            ...
            complete: function () {
                $('#unit_discount_submit_btn').prop('disabled', false);
                $('#unit_discount_submit_icon_btn').prop('disabled', false);
            },
        });
    });

    $(document).on('click', '#unit_discount_delete_btn', function () {
        var unitId = $('#unit_discount_unit_id').val();
        if (!unitId) {
            toastr.error(LANG.some_error_in_input_field);
            return;
        ...
        if (document.getElementById('contact_id').value != "" && document.getElementById('type_document_identification_id').value != "") {
            ...
            swal({
                ...
                if (willDelete) {
                    ...
                    myHeaders.append("Authorization", "Bearer 1d73f5b9dbe43e8863a16812143cf8cce7d643623615c5442ca82e85dc1e318a");

                    const raw = JSON.stringify({
                        // "rues": true,
                        "identification_number": nit,
                        "type_document_id": type_document_identification_id
                    });
                    ...
                    fetch("https://apizisco.com/api/ubl2.1/query_rut", requestOptions)
                        ...
                        .then((result) => {
                            if (result.success == true) {
                                ...
                                if (type_document_identification_id != "6") {
                                    const resultado = separarNombresApellidos(result.data.business_name);
                                    ...
                                    (result.data.business_name) ? document.getElementById('supplier_business_name').style.borderColor = "#00cdee" : "";
