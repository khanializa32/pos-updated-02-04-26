...
<img src="@if(count($product->media) > 0)
		...
		<input type="hidden" name="products[{{$row_count}}][line_location_id]" class="line_location_id" value="{{ $location_id ?? '' }}">
		<input type="hidden" 
			class="product_type" 
	...
	<td>
		...
		@if(count($sub_units) > 0)
            ...
            <select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit">
                <option value="" data-multiplier="1" data-unit_name="" data-allow_decimal="1">Presentacion</option>
                @foreach($sub_units as $key => $value)
