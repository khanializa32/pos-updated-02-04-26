...
<tr class="product_row" data-row_index="{{$row_count}}" data-unit-discount="{{ $product->unit_discount ?? '' }}"
	...
	<td>
		...
		<input type="hidden" name="products[{{$row_count}}][line_location_id]" class="line_location_id"
			value="{{ $transactionLocation->id ?? '' }}">
    ...
    @endif
		...
		@if(session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1)
			...
			@if(!empty($product->lot_numbers) && empty($is_sales_order))
				@if($lot_enabled == 1)
					{{-- Manual lot selection: shown when lot number tracking is enabled --}}
					<select class="form-control lot_number input-sm" name="products[{{$row_count}}][lot_no_line_id]"
						...
						@foreach($product->lot_numbers as $lot_number)
							...
							<option value="{{$lot_number->purchase_line_id}}" data-qty_available="{{$lot_number->qty_available}}"
								...
								{{@format_date($lot_number->exp_date)}} @endif {{$expiry_text}}
							</option>
						@endforeach
					</select>
				@else
					{{-- FEFO mode: server auto-selects batch by expiry date; show read-only info only --}}
					@php
					...
					@if(!empty($fefo_batch))
						<small class="text-muted">
							@if($exp_enabled == 1 && !empty($fefo_batch->exp_date))
								@php
									$fefo_is_expired = \Carbon::now()->gt(\Carbon::createFromFormat('Y-m-d', $fefo_batch->exp_date));
								@endphp
								<i class="fa fa-calendar"></i>
								@lang('product.exp_date'): {{ @format_date($fefo_batch->exp_date) }}
								@if($fefo_is_expired)
									<span class="label label-danger">@lang('report.expired')</span>
								@endif
							@elseif($exp_enabled == 1)
								<i class="fa fa-calendar"></i> @lang('lang_v1.no_expiry_date')
							@endif
						</small>
					@endif
				@endif
	...
	<td>
		...
		<div class="input-group input-number">
			...
			<input type="text" data-min="1" style="width: 6vh; padding: 0px 2px 0px 4px"
				...
				data-enable-stock="{{ $product->enable_stock ?? '' }}"
		...
		{{-- NUEVA ESTRUCTURA: Selector y Basura alineados --}}
		<div style="display: flex; align-items: center; gap: 5px; margin-top: 5px;">
			<input type="hidden" name="products[{{$row_count}}][product_unit_id]" value="{{$product->unit_id}}">
			@if(count($sub_units) > 0)
				<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit" style="width: 100%;">
					...
					@foreach($sub_units as $key => $value)
						<option value="{{$key}}" data-multiplier="{{$value['multiplier']}}" data-unit_name="{{$value['name']}}"
							data-allow_decimal="{{$value['allow_decimal']}}" data-price="{{$value['price'] ?? ''}}"
							data-cost-price="{{$value['cost_price'] ?? ''}}" @if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key) selected @endif>
							{{$value['name']}}
						</option>
					@endforeach
				</select>
		...
		<input type="hidden" class="hidden_base_unit_sell_price_inc_tax"
			value="{{$product->sell_price_inc_tax / $multiplier}}">
		@if(empty($is_direct_sell))
			{!! Form::select("products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount_type, ['class' => 'form-control row_discount_type', 'style' => 'display:none;']) !!}
			...
			{!! Form::select("products[$row_count][tax_id]", $tax_dropdown['tax_rates'], $tax_id, ['placeholder' => 'Select', 'class' => 'form-control tax_id', 'style' => 'display:none;'], $tax_dropdown['attributes']) !!}
		@endif
	...
	@if(!empty($is_direct_sell))
		...
		<td class="@if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
			<input type="text" name="products[{{$row_count}}][unit_price]"
				...
				data-rule-min-value="{{$product->default_purchase_price}}"
				data-msg-min-value="{{__('lang_v1.cost_price_validation_error', ['cost_price' => @num_format($product->default_purchase_price)])}}"
				data-cost-price="{{$product->default_purchase_price}}">
	...
	@endif
	<td class="{{$hide_tax}}">
		<input type="text" style="width: 10vh" name="products[{{$row_count}}][unit_price_inc_tax]"
			class="form-control pos_unit_price_inc_tax input_number" value="{{@num_format($unit_price_inc_tax)}}"
			@if(!$edit_price) readonly @endif data-rule-min-value="{{$product->default_purchase_price}}"
			data-msg-min-value="{{__('lang_v1.cost_price_validation_error', ['cost_price' => @num_format($product->default_purchase_price)])}}"
			data-cost-price="{{$product->default_purchase_price}}">
	</td>
	...
	<td class="text-center">
		<input type="hidden" style="width: auto" name="products[{{$row_count}}][unit_price_inc_tax]"
			class="form-control pos_unit_price_inc_tax input_number" value="{{@num_format($unit_price_inc_tax)}}"
			@if(!$edit_price) readonly @endif data-rule-min-value="{{$product->default_purchase_price}}"
			data-msg-min-value="{{__('lang_v1.cost_price_validation_error', ['cost_price' => @num_format($product->default_purchase_price)])}}"
			data-cost-price="{{$product->default_purchase_price}}">
		...
		@endphp
		<input style="width: 9vh" type="{{$subtotal_type}}"
